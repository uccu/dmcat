...
class Smtp
{
    ...
    function __construct($relay_host = "", $smtp_port = 25, $auth = false, $user, $pass)
    {
        ...
        $this->time_out = 100; //is used in fsockopen() 
        $this->auth = $auth; //auth
        $this->user = $user;
        $this->pass = $pass;
        $this->host_name = "4moe.com"; //is used in HELO command 
        $this->log_file = "";
        $this->sock = FALSE;
    }
    ...
    function sendmail($to, $name, $from, $subject = "", $body = "", $mailtype, $cc = "", $bcc = "", $additional_headers = "")
    {
    ...
    {
        return $this->relay_host == "" ? $this->smtp_sockopen_mx($address) : $this->smtp_sockopen_relay();
    }
    ...
    static function send($smtpemailto = '418667631@qq.com', $mailtitle = '邮件测试', $mailcontent = '邮件测试', $mailtype = "HTML")
    {
        $smtpserver = "smtp.ym.163.com"; //SMTP服务器
        $smtpserverport = 25; //SMTP服务器端口
        $smtpusermail = "i@4moe.com"; //SMTP服务器的用户邮箱
        $smtpusername = 'Y4';
        //发送给谁
        $smtpuser = "i@4moe.com"; //SMTP服务器的用户帐号
        $smtppass = "9y5xlfYzxf"; //SMTP服务器的用户密码
        //邮件主题
        //邮件内容
        ...
        static $object;
        if (empty($object)) {
            $object = new self($smtpserver, $smtpserverport, true, $smtpuser, $smtppass);
        }
        //这里面的一个true是表示使用身份验证,否则不使用身份验证.
        //$object->debug = true;//是否显示发送的调试信息
        $state = $object->sendmail($smtpemailto, $smtpusername, $smtpusermail, $mailtitle, $mailcontent, $mailtype);
